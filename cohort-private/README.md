...
- Per-learner access tokens and personal dashboard URLs
...
## Per-student dashboard URLs

Each learner gets one private URL of the form:

```
https://YOUR-HOST/?t=<long-random-token>
```

Anyone with the link sees that learner's quiz scores and submitted assignments on the course home table. Treat each URL like a password and share it directly with that one learner.

Older handouts that used `/progress?t=...` still reach the same dashboard (the server redirects to `/?t=...`).

### Same-browser session (for instructors)

After a learner opens **any** course URL that includes `?t=<their-token>`, the site may set a short-lived **HttpOnly cookie** so they stay identified while navigating (site header, home, lessons, quizzes) without `t` on every link. They still need their tokenized URL again on a **new device** or if they clear cookies. This does **not** replace the private dashboard URL for support or bookmarks.
...
The script prints CSV rows (`display_name,access_token,dashboard_url`). Save them to `cohort-private/student-access-tokens.csv` (gitignored).
...
Share each `dashboard_url` with that learner only (DM, email, your LMS). Do not paste the full sheet anywhere classmates can see other rows.
...
Quiz attempts and assignment submissions made before student tokens existed have `student_id = NULL` and will not appear on a learner's dashboard. If a roster's display names match the historical free-text names exactly (case-insensitive), you can attach those rows by running:
